...
import (
	...
	"github.com/TimaSlipko/gomobile/bind"
	"github.com/TimaSlipko/gomobile/internal/importers"
	"github.com/TimaSlipko/gomobile/internal/importers/java"
	"github.com/TimaSlipko/gomobile/internal/importers/objc"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		if p == nil {
			dir, err := packageDir("github.com/TimaSlipko/gomobile/bind")
			if err != nil {
				errorf(`"github.com/TimaSlipko/gomobile/bind" is not found; run go get github.com/TimaSlipko/gomobile/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(dir, "..")) // github.com/TimaSlipko/gomobile directory.
			...
			javaDir, err := packageDir("github.com/TimaSlipko/gomobile/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/TimaSlipko/gomobile/bind/objc")
			if err != nil {
