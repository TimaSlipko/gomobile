...
func runBuildImpl(cmd *command) (*packages.Package, error) {
	...
	if !nmpkgs["github.com/TimaSlipko/gomobile/app"] {
		return nil, fmt.Errorf(`%s does not import "github.com/TimaSlipko/gomobile/app"`, pkg.PkgPath)
	}
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/TimaSlipko/gomobile/app": true}, nil
	}
	r, w := io.Pipe()
